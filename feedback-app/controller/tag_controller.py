...
def get_all_tags() -> Response:
    ...
    if tags is None:
        msg = json.dumps({"message": "невозможно получить список тегов"}, ensure_ascii=False)
...
def get_one_metric(id: int) -> Response:
    ...
    if tag is None:
        msg = json.dumps({"message": "тег с данным id отсутсвует"}, ensure_ascii=False)
...
def add_tag() -> Response:
    ...
    if title is None or type(title) is not str:
        msg = json.dumps({"message": "title должен быть валидной строкой"}, ensure_ascii=False)
        return Response(msg, status=422, mimetype='application/json')
    if typed is None or type(typed) is not str:
        msg = json.dumps({"message": "type должен быть валидной строкой"}, ensure_ascii=False)
        return Response(msg, status=422, mimetype='application/json')
    ...
    if tag is None:
        msg = json.dumps({"message": "тег с данным id отсутствует"}, ensure_ascii=False)
        return Response(msg, status=422, mimetype='application/json')
